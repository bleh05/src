...
void timer_sleep(int64_t ticks) {
    ...
    // printf("%llx LOCK ADDY \n", sema);
    struct list_elem *elem = malloc(sizeof(struct list_elem));
    struct timer *tim = malloc(sizeof(struct timer));
    tim->elem = *elem;
    tim->sema = *sema;
    sema_init(&tim->sema, 0);
    tim->ticks = ticks;
    tim->id = ticks;
    list_push_back(&timer_list, &tim->elem);
    sema_down(&tim->sema);
...
static void timer_interrupt(struct intr_frame *args UNUSED) {
    ticks++;
    struct list_elem *e;
    for (e = list_rbegin(&timer_list); e != list_rend(&timer_list); e = list_prev(e)) {
        struct timer *timer = list_entry(e, struct timer, elem);
        timer->ticks--;
        if (timer->ticks <= 0) {
            list_remove(e);
            sema_up(&timer->sema);
